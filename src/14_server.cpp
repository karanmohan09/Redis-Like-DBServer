...
const uint32_t k_rdb_version = 1; // version of the RDB format

volatile bool g_running = true;
static void save_db(const char *filename);
static void load_db(const char *filename);
...
int main() {
    ...
    load_db("dump.rdb"); //load db at startup
    ...
    if(epoll_ctl(epfd, EPOLL_CTL_ADD, fd, &ev) < 0) {
        die("epoll_ctl listen fd");
    }
    // the event loop
    signal(SIGINT, handle_sigint);  // Catch Ctrl+C
    while (g_running) {
        int timeout = next_timer_ms();
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (conn->want_close) {
                ...
                cev.data.fd = conn->fd;
                cev.events = 0;
                if (conn->want_read)  cev.events |= EPOLLIN;
                if (conn->want_write) cev.events |= EPOLLOUT;
        ...
        while (!dlist_empty(&g_data.idle_list)) {
            DList *idle_node = g_data.idle_list.next;
            Conn *conn = container_of(idle_node, Conn, idle_node);
            int64_t idle_ms = now - conn->last_active_ms;
            if (idle_ms < k_idle_timeout_ms) break;
            epoll_ctl(epfd, EPOLL_CTL_DEL, conn->fd, NULL);
            conn_destroy(conn);
        }
    }   // the event loop
    save_db("dump.rdb"); // save db at shutdown
    return 0;
}
...
static void save_db(const char *filename) {
    ...
    fwrite(&k_rdb_version, sizeof(k_rdb_version), 1, fp);
    ...
    hm_foreach(&g_data.db, [](HNode *node, void *arg) -> bool {
        ...
        uint32_t keylen = (uint32_t)ent->key.size();
        fwrite(&keylen, sizeof(keylen), 1, fp);
        fwrite(ent->key.data(), 1, keylen, fp);

        fwrite(&ent->type, sizeof(ent->type), 1, fp);

        // Write TTL if present
        bool has_ttl = (ent->heap_idx != (size_t)-1);
        fwrite(&has_ttl, sizeof(has_ttl), 1, fp);
        if (has_ttl) {
            uint64_t expire_at = g_data.heap[ent->heap_idx].val;
            fwrite(&expire_at, sizeof(expire_at), 1, fp);
        }

        if (ent->type == T_STR) {
            uint32_t slen = (uint32_t)ent->str.size();
            fwrite(&slen, sizeof(slen), 1, fp);
            fwrite(ent->str.data(), 1, slen, fp);
        } else if (ent->type == T_ZSET) {
            uint32_t zsize = (uint32_t)hm_size(&ent->zset.hmap);
            fwrite(&zsize, sizeof(zsize), 1, fp);
            hm_foreach(&ent->zset.hmap, [](HNode *hnode, void *arg) -> bool {
                FILE *fp = (FILE *)arg;
                ZNode *znode = container_of(hnode, ZNode, hmap);
                uint32_t nlen = (uint32_t)znode->len;
                fwrite(&nlen, sizeof(nlen), 1, fp);
                fwrite(znode->name, 1, nlen, fp);
                fwrite(&znode->score, sizeof(znode->score), 1, fp);
                return true;
            }, fp);
        }
        return true;
...
static void load_db(const char *filename) {
    ...
    // Clear old DB
    hm_foreach(&g_data.db, [](HNode *node, void *)->bool {
    Entry *ent = container_of(node,Entry,node);
    entry_del(ent);
    return true;
    },nullptr);
    ...
    uint32_t version = 0;
    if (fread(&version, sizeof(version), 1, fp) != 1 || version != k_rdb_version) {
        fclose(fp);
        fprintf(stderr, "Invalid or unsupported dump file version.\n");
        return;
    }
    ...
    for (uint32_t i = 0; i < n; ++i) {
        uint32_t keylen = 0;
        if (fread(&keylen, sizeof(keylen), 1, fp) != 1) break;
        std::string key(keylen, '\0');
        if (fread(&key[0], 1, keylen, fp) != keylen) break;

        uint32_t type = 0;
        if (fread(&type, sizeof(type), 1, fp) != 1) break;

        bool has_ttl = false;
        if (fread(&has_ttl, sizeof(has_ttl), 1, fp) != 1) break;
        ...
        if (type == T_STR) {
            uint32_t slen = 0;
            if (fread(&slen, sizeof(slen), 1, fp) != 1) break;
            std::string val(slen, '\0');
            if (fread(&val[0], 1, slen, fp) != slen) break;
            std::vector<std::string> cmd = {"set", key, val};
            Buffer dummy;
            do_set(cmd, dummy);
        } else if (type == T_ZSET) {
            uint32_t zsize = 0;
            if (fread(&zsize, sizeof(zsize), 1, fp) != 1) break;
            for (uint32_t j = 0; j < zsize; ++j) {
                uint32_t nlen = 0;
                if (fread(&nlen, sizeof(nlen), 1, fp) != 1) break;
                std::string name(nlen, '\0');
                if (fread(&name[0], 1, nlen, fp) != nlen) break;
                double score = 0;
                if (fread(&score, sizeof(score), 1, fp) != 1) break;
                std::vector<std::string> cmd = {"zadd", key, std::to_string(score), name};
                Buffer dummy;
                do_zadd(cmd, dummy);
            }
        }
        ...
        // Restore TTL
        if (has_ttl) {
            int64_t ttl_ms = (int64_t)expire_at - get_monotonic_msec();
            if (ttl_ms > 0) {
                std::vector<std::string> cmd = {"pexpire", key, std::to_string(ttl_ms)};
                Buffer dummy;
                do_expire(cmd, dummy);
            }
        }
